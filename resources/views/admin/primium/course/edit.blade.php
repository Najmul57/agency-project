...
@section('admin_content')
    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ route('primium.course.update', $data->id) }}" method="post" enctype="multipart/form-data">
                @csrf
                ...
                @php
                    $department = \App\models\DepartmentList::latest()->get();
                @endphp
                <div class="form-group mb-3">
                    <label for="name"><strong>Name</strong></label>
                    <select name="name" class="select2 form-control @error('name') is-invalid @enderror" id="">
                        ...
                        @foreach ($department as $item)
                            <option value="{{ $item->name }}" {{ $item->name == $data->name ? 'selected' : '' }}>
    ...
    <script>
        $(document).ready(function() {
            // Fetch the selected country ID from the server-side data
            var selectedCountryId = "{{ $data->country_id ?? '' }}";
            // Fetch the selected university ID from the server-side data
            var selectedUniversityId = "{{ $data->university_id ?? '' }}";
            // Fetch the selected program type ID from the server-side data
            var selectedProgramTypeId = "{{ $data->program_type_id ?? '' }}";
                ...
                if (countryId) {
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            // Select the previously selected university
            ...
            // Fetch universities and select the appropriate one on page load
                ...
                if (universityId) {
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            // Select the previously selected program type
            ...
            // Fetch program types and select the appropriate one on page load
